...
def get_parser() -> argparse.ArgumentParser:
    ...
    ## add flexibility to use AOAI endpoint
...
class ModelResolver:
    ...
    def _resolve_api_version(self, model_name: str) -> str:
        if model_name.startswith("gpt-35-turbo") or model_name.startswith("gpt-4"):
            return "2023-03-15-preview"
        return "2022-12-01"

    def resolve(self, name: str, api_base: str) -> ModelSpec:
        if "aoai" in api_base:
            openai.api_type = "azure"
            openai.api_base = api_base
            openai.api_version = self._resolve_api_version(name)
        if name in self.DUMMY_MODELS:
            result = ModelSpec(name=name, model=name, is_chat=(name in self.CHAT_MODELS), api_base=api_base, engine=name)
            return result
...
def run(args, model_resolver: ModelResolver, registry: Optional[Registry] = None):
    ...
    def get_model(name: str, api_base: str) -> ModelSpec:
        return model_resolver.resolve(name, api_base)

    completion_model_specs = [get_model(model, args.api_base) for model in args.model.split(",")]
    ...
    model_specs = ModelSpecs(
        ...
        embedding_=get_model(args.embedding_model, args.api_base) if args.embedding_model else None,
        ranking_=get_model(args.ranking_model, args.api_base) if args.ranking_model else None,
    )
